...
exports.exists = exports.analyze = void 0;
...
async function findTargets(currentDir, targets, sourceId, depth) {
    await Promise.all(Array.from(targets).map(async ([target, added]) => {
        if (!added) {
            ...
            const targetId = await getTargetId(targetName, targetVersion, targetDir);
            if (targetId) {
...
async function parseDependencies(sourceId, dependencies, rootDir, parentDir, depth) {
...
async function parsePackageJson(dir, rootDir, depth) {
    if (depth < 1)
        return;
    ...
    if (dependencies)
        await parseDependencies(nodeId, dependencies, rootDir, dir, depth - 1);
    parsedNodes.add(nodeId);
}
// Function to perform a breadth-first search starting from the root directory.
async function breadthFirstSearch(rootDir) {
...
async function analyze(dir, depth = 5) {
